...
function rbCheck(root) {
  ...
  function walk(node) {
    ...
    return {
      ok: left.ok && right.ok,
      blackHeight: left.blackHeight + (node.color === false ? 1 : 0),
    };
  }
  return walk(root).ok;
}
...
test("graph algorithms produce valid canonical results", () => {
  assert.deepEqual(AlgorithmCore.graphs.run("bfs", "A", "G").path, [
    "A",
    "C",
    "E",
    "G",
  ...
  assert.deepEqual(AlgorithmCore.graphs.run("topo").order, [
    "A",
    "B",
    "C",
    "D",
  ...
  const cycleGraph = AlgorithmCore.graphs.graphFrom(
    AlgorithmCore.graphs.samples.negativeCycle,
  );
  assert.equal(
    AlgorithmCore.graphs.run("bellman", "A", "C", cycleGraph).negativeCycle,
    true,
  );
  assert.equal(AlgorithmCore.graphs.run("prim", "A").total, 14);
  assert.equal(AlgorithmCore.graphs.run("kruskal").total, 14);
  assert.deepEqual(AlgorithmCore.graphs.run("scc").components, [
    ["A", "B", "C", "D"],
    ["E", "F"],
  ]);
});
...
test("tree sessions preserve expected invariants", () => {
  for (const key of ["bst", "avl", "rb", "splay", "treap"]) {
    ...
    assert.deepEqual(
      inorder(snap.root),
      AlgorithmCore.util.sorted(session.keys),
      key,
    );
  ...
  for (const key of ["twoThree", "twoThreeFour", "btree", "bplus"]) {
    ...
    for (const node of snap.levels.flat())
      assert.ok(node.keys.length <= maxKeys, key);
    assert.deepEqual(
      AlgorithmCore.trees.invariants(key, snap).keys,
      AlgorithmCore.util.sorted(session.keys),
    );
  }
});
...
test("optional visualizer sizing is deterministic and backward compatible", () => {
  ...
  const graph = AlgorithmCore.graphs.graphFrom(
    AlgorithmCore.graphs.samples.weighted,
    { size: 12 },
  );
  assert.equal(graph.nodes.length, 12);
  assert.ok(
    AlgorithmCore.graphs.run("dijkstra", "A", "L", graph).distances.L <
      Infinity,
  );
...
test("randomized visualizer data stays valid and differs from canonical samples", () => {
  const coin = AlgorithmCore.dp.run("coin", {
    size: 8,
    randomize: true,
    rng: seeded(1),
  });
  ...
  const lis = AlgorithmCore.dp.run("lis", {
    size: 10,
    randomize: true,
    rng: seeded(2),
  });
  ...
  const huffman = AlgorithmCore.greedy.run("huffman", {
    size: 6,
    randomize: true,
    rng: seeded(3),
  });
  ...
  const tree = AlgorithmCore.trees.createSession("avl", {
    size: 12,
    randomize: true,
    rng: seeded(4),
  });
  ...
  assert.deepEqual(
    inorder(treeSnap.root),
    AlgorithmCore.util.sorted(tree.keys),
  );
  assert.equal(avlBalanced(treeSnap.root).ok, true);
  assert.notDeepEqual(
    tree.keys,
    [50, 25, 75, 12, 37, 62, 88, 31, 43, 57, 70, 77],
  );

  const graph = AlgorithmCore.graphs.graphFrom(
    AlgorithmCore.graphs.samples.weighted,
    { size: 9, randomize: true, rng: seeded(5) },
  );
  assert.equal(graph.nodes.length, 9);
  assert.ok(graph.edges.every((edge) => edge.weight > 0));
  assert.ok(
    AlgorithmCore.graphs.run("dijkstra", "A", "I", graph).distances.I <
      Infinity,
  );

  const negativeGraph = AlgorithmCore.graphs.graphFrom(
    AlgorithmCore.graphs.samples.negative,
    { size: 7, randomize: true, rng: seeded(7) },
  );
  assert.equal(
    AlgorithmCore.graphs.run("bellman", "A", "G", negativeGraph).negativeCycle,
    false,
  );

  const topic = AlgorithmCore.topics.run("searching", "binary", {
    size: 9,
    randomize: true,
    rng: seeded(6),
  });
  const values = topic.steps[0].items.map((item) =>
    Number(item.label.split(":")[1]),
  );
  assert.deepEqual(values, AlgorithmCore.util.sorted(values));
  assert.ok(topic.answer >= 0 && topic.answer < values.length);
});

test("custom dynamic programming inputs are honored", () => {
  assert.equal(AlgorithmCore.dp.run("fib", { n: 10 }).answer, 55);
  assert.equal(
    AlgorithmCore.dp.run("coin", { coins: [1, 3, 4], amount: 6 }).answer,
    2,
  );
  assert.equal(AlgorithmCore.dp.run("lcs", { x: "ABC", y: "AC" }).answer, 2);
  assert.equal(
    AlgorithmCore.dp.run("edit", { a: "gumbo", b: "gambol" }).answer,
    2,
  );
  assert.equal(
    AlgorithmCore.dp.run("lis", { input: [3, 10, 2, 1, 20] }).answer,
    3,
  );
});

test("custom greedy inputs are honored", () => {
  const activity = AlgorithmCore.greedy.run("activity", {
    activities: [
      { id: "A1", s: 0, f: 2 },
      { id: "A2", s: 1, f: 3 },
      { id: "A3", s: 3, f: 5 },
  ...
  assert.deepEqual(activity.answer, ["A1", "A3"]);

  const fractional = AlgorithmCore.greedy.run("fractional", {
    capacity: 50,
    items: [
      { id: "A", w: 10, v: 60 },
      { id: "B", w: 20, v: 100 },
  ...
  assert.equal(fractional.value, 240);
  ...
  assert.equal(huffman.value, 26);
});

test("custom topic inputs drive search strings heaps and geometry", () => {
  assert.equal(
    AlgorithmCore.topics.run("searching", "linear", {
      values: [8, 5, 13],
      target: 13,
    }).answer,
    2,
  ...
  assert.equal(
    AlgorithmCore.topics.run("searching", "binary", {
      values: [1, 3, 5, 8, 13],
      target: 8,
    }).answer,
    3,
  ...
  assert.equal(
    AlgorithmCore.topics
      .run("strings", "naive", { text: "CABABA", pattern: "ABA" })
      .answer.join(","),
    "1,3",
  );
  assert.deepEqual(
    AlgorithmCore.topics.run("heaps", "insert", {
      heap: [2, 4, 6],
      keyValue: 1,
    }).answer,
    [2, 1, 6, 4],
  ...
  assert.equal(
    AlgorithmCore.topics.run("geometry", "closest", {
      points: [
        { x: 0, y: 0 },
        { x: 3, y: 4 },
      ],
...
test("manual validation helpers can detect sortedness-sensitive invalid input", () => {
  assert.equal(AlgorithmCore.util.isSortedAscending([1, 2, 2, 5]), true);
  assert.equal(AlgorithmCore.util.isSortedAscending([1, 5, 3]), false);
});
