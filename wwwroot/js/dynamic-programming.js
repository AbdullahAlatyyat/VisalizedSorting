...
const DP_ALGORITHMS = {
  fib: {
    ...
    answer: "21",
    description:
      "Shows Fibonacci as overlapping subproblems: memoization stores recursive results, while tabulation fills the same states from the bottom up.",
    pseudocode:
      "dp[0] = 0\ndp[1] = 1\nfor i from 2 to n:\n  dp[i] = dp[i - 1] + dp[i - 2]",
  },
  coin: {
    ...
    answer: "3",
    description:
      "Finds the minimum number of coins for each amount by reusing smaller optimal amounts.",
    pseudocode:
  ...
  knapsack: {
    ...
    answer: "9",
    description:
      "Chooses whether to skip or take each item. Each cell asks: best value using the first i items and capacity w.",
    pseudocode:
  ...
  lcs: {
    ...
    time: "O(mn)",
    space: "O(mn)",
    example: "ABCBDAB vs BDCABA",
    answer: "4",
    description:
      "Compares prefixes of two strings. Matching characters extend a subsequence; mismatches inherit the best neighboring prefix.",
    pseudocode:
  ...
  edit: {
    ...
    answer: "3",
    description:
      "Computes the cheapest sequence of insertions, deletions, and substitutions to transform one string into another.",
    pseudocode:
      "if a[i] == b[j]: cost = 0\nelse: cost = 1\ndp[i][j] = min(\n  dp[i - 1][j] + 1,\n  dp[i][j - 1] + 1,\n  dp[i - 1][j - 1] + cost)",
  },
  matrix: {
    ...
    time: "O(n^3)",
    space: "O(n^2)",
    example: "10x30, 30x5, 5x60",
    answer: "4500",
    description:
      "Finds the cheapest parenthesization by splitting every interval at every possible midpoint.",
    pseudocode:
  ...
  lis: {
    ...
    answer: "4",
    description:
      "Tracks the best increasing subsequence ending at each index by looking back at all smaller previous values.",
    pseudocode:
...
const DP_INPUTS = {
  fib: { fields: [{ name: "n", label: "n", value: "8", hint: "2-20" }] },
  coin: {
    fields: [
      { name: "coins", label: "Coins", value: "1,2,5" },
      { name: "amount", label: "Amount", value: "11" },
    ],
  },
  knapsack: {
    fields: [
      { name: "items", label: "Items w:v", value: "1:1,3:4,4:5,5:7" },
      { name: "capacity", label: "Capacity", value: "7" },
    ],
  },
  lcs: {
    fields: [
      { name: "x", label: "First String", value: "ABCBDAB" },
      { name: "y", label: "Second String", value: "BDCABA" },
    ],
  },
  edit: {
    fields: [
      { name: "a", label: "From", value: "kitten" },
      { name: "b", label: "To", value: "sitting" },
    ],
  },
  matrix: {
    fields: [{ name: "dimensions", label: "Dimensions", value: "10,30,5,60" }],
  },
  lis: {
    fields: [{ name: "input", label: "Array", value: "10,9,2,5,3,7,101,18" }],
  },
};
...
function pairsFrom(value, leftName, rightName) {
  ...
  return parts.map((part) => {
    ...
    if (!Number.isFinite(left) || !Number.isFinite(right))
      throw new Error("Use pairs like 3:4,5:7.");
    return { [leftName]: left, [rightName]: right };
  });
}
...
function renderInputPanel() {
  ensureInputPanel();
  const config = DP_INPUTS[currentKey];
            ...
            ${config.fields
              .map(
                ...
                <label class="vs-input-label">
                    ...
                    <input class="vs-manual-input" data-input="${field.name}" value="${field.value}" ${field.hint ? `placeholder="${field.hint}"` : ""} />
                </label>
            `,
              )
              .join("")}
...
function collectManualOptions() {
  if (currentKey === "fib")
    return { n: boundedNumber(inputValue("n"), "n", 2, 20) };
  if (currentKey === "coin") {
    const coins = numbersFrom(inputValue("coins"))
      .map(Math.round)
      .filter((value) => value > 0);
    if (!coins.length) throw new Error("Coins must include positive values.");
    return {
      coins,
      amount: boundedNumber(inputValue("amount"), "Amount", 0, 60),
    };
  }
  if (currentKey === "knapsack")
    return {
      items: pairsFrom(inputValue("items"), "w", "v"),
      capacity: boundedNumber(inputValue("capacity"), "Capacity", 1, 60),
    };
  if (currentKey === "lcs") {
    const x = inputValue("x"),
      y = inputValue("y");
    if (!x || !y) throw new Error("Both strings are required.");
    return { x, y };
  }
  if (currentKey === "edit") {
    const a = inputValue("a"),
      b = inputValue("b");
    if (!a || !b) throw new Error("Both strings are required.");
    return { a, b };
  }
  if (currentKey === "matrix") {
    const dimensions = numbersFrom(inputValue("dimensions")).map(Math.round);
    if (
      dimensions.length < 3 ||
      dimensions.length > 8 ||
      dimensions.some((value) => value <= 0)
    )
      throw new Error("Enter 3 to 8 positive dimensions.");
    return { dimensions };
  }
  if (currentKey === "lis") {
    const input = numbersFrom(inputValue("input")).map(Math.round);
    if (input.length < 2 || input.length > 20)
      throw new Error("Array must contain 2 to 20 numbers.");
    return { input };
  }
  return {};
}
...
function renderGrid() {
  refs.grid.innerHTML = Object.entries(DP_ALGORITHMS)
    .map(
      ([key, algo]) => `
...
function buildSteps() {
  pause();
  refs.log.innerHTML = "";
  const result = (RUNNERS[currentKey] || runFib)(problemSize(), runOptions());
...
function emptyRows(r, c, fill = "") {
  return Array.from({ length: r }, () => Array(c).fill(fill));
}
function tableStep(message, table) {
  return { message, table };
}
function keyOf(r, c) {
  return `${r}:${c}`;
}
function resultOf(steps, example, answer) {
  return { steps, example, answer };
}
...
function coreDp(key, size, options) {
  return globalThis.AlgorithmCore
    ? globalThis.AlgorithmCore.dp.run(key, { size, ...(options || {}) })
    : null;
}
...
function renderTable(table) {
  ...
  colLabels.forEach((label) =>
    cells.push(`<div class="vs-dp-cell vs-dp-cell-label">${label}</div>`),
  );
  rows.forEach((row, r) => {
    cells.push(
      `<div class="vs-dp-cell vs-dp-cell-label">${rowLabels[r] ?? r}</div>`,
    );
    row.forEach((value, c) => {
      const cls = active.has(keyOf(r, c))
        ? "vs-dp-cell-write"
        : done.has(keyOf(r, c))
          ? "vs-dp-cell-done"
          : "";
...
function runFib(size = 8, options = {}) {
  if (globalThis.AlgorithmCore) {
    ...
    const mapped = core.steps.map((step) => {
      const rows = [
        step.state.memo.map((v) => v ?? ""),
        step.state.tab.map((v) => v ?? ""),
      ];
      const done = [];
      rows.forEach((row, r) =>
        row.forEach((value, c) => {
          if (value !== "") done.push([r, c]);
        }),
      );
      const active = step.state.active.flatMap((i) => [
        [0, i],
        [1, i],
      ]);
      return tableStep(
        ...
        makeTable(
          ...
          ["memo", "tab"],
          labels,
          "Memoization caches recursive calls; tabulation fills states bottom-up.",
          active,
          done,
        ),
      );
  ...
  const out = [
    tableStep(
      "Compare memoized states with bottom-up table states",
      makeTable(
        rows,
        ["memo", "tab"],
        [...Array(n + 1).keys()],
        `Fibonacci states F(0) through F(${n}).`,
        [],
        done,
      ),
    ),
  ...
  out.push(
    tableStep(
      ...
      makeTable(
        ...
        ["memo", "tab"],
        [...Array(n + 1).keys()],
        "Both approaches store the same base states.",
        [
          [0, 0],
          [0, 1],
          [1, 0],
  ...
  for (let i = 2; i <= n; i++) {
    ...
    out.push(
      tableStep(
        `Fill F(${i}) = F(${i - 1}) + F(${i - 2}) = ${rows[1][i]}`,
        makeTable(
          rows,
          ["memo", "tab"],
          [...Array(n + 1).keys()],
          "Memoization and tabulation converge to the same cached values.",
          [
            [0, i],
            [1, i],
          ],
...
function runCoin(size = 8, options = {}) {
  ...
  const coins = core ? core.coins : [1, 2, 5],
    amount = core ? core.table.length - 1 : size + 3;
  ...
  out.push(
    tableStep(
      "dp[0] = 0 because zero coins make amount 0",
      makeTable(
        [dp],
        ["min coins"],
        [...Array(amount + 1).keys()],
  ...
  for (let a = 1; a <= amount; a++) {
    ...
    out.push(
      tableStep(
        `Best for amount ${a} is ${dp[a]}`,
        makeTable(
          [dp],
          ["min coins"],
          [...Array(amount + 1).keys()],
          "Each amount reuses earlier optimal amounts.",
          [[0, a]],
          [...done],
        ),
      ),
...
function runKnapsack(size = 8, options = {}) {
  ...
  const itemCount = Math.max(
    3,
    Math.min(baseItems.length, Math.round(size / 2)),
  );
  ...
  const out = [
    tableStep(
      "Initialize row 0: no items means value 0",
      makeTable(
        rows,
        rowLabels,
        [...Array(cap + 1).keys()],
        `Items: ${items.map((i) => `(w${i.w},v${i.v})`).join(", ")}.`,
        [],
        done,
      ),
    ),
  ...
  for (let i = 1; i <= items.length; i++) {
    for (let w = 0; w <= cap; w++) {
      ...
      out.push(
        tableStep(
          `Item ${i}, capacity ${w}: max(skip ${skip}, take ${take === -Infinity ? "n/a" : take}) = ${rows[i][w]}`,
          makeTable(
            rows,
            rowLabels,
            [...Array(cap + 1).keys()],
            "Rows are item prefixes; columns are capacities.",
            [[i, w]],
            [...done],
          ),
        ),
  ...
  return resultOf(
    out,
    `capacity ${cap}; ${items.length} items`,
    rows[items.length][cap],
  );
}
...
function runLcs(size = 8, options = {}) {
  ...
  const out = [
    tableStep(
      "Initialize empty-prefix row and column to 0",
      makeTable(
        rows,
        ["-", ...x],
        ["-", ...y],
        "dp[i][j] is LCS length for prefixes x[0..i) and y[0..j).",
        [],
        done,
      ),
    ),
  ...
  for (let i = 1; i <= x.length; i++) {
    for (let j = 1; j <= y.length; j++) {
      ...
      out.push(
        tableStep(
          `${x[i - 1]} vs ${y[j - 1]} -> dp[${i}][${j}] = ${rows[i][j]}`,
          makeTable(
            rows,
            ["-", ...x],
            ["-", ...y],
            "Matches extend diagonally; mismatches copy the best neighbor.",
            [[i, j]],
            [...done],
          ),
        ),
...
function runEdit(size = 8, options = {}) {
  ...
  const out = [
    tableStep(
      "Initialize insertion/deletion cost against empty prefixes",
      makeTable(
        rows,
        ["-", ...a],
        ["-", ...b],
        "Cells store edit distance between prefixes.",
        [],
        done,
      ),
    ),
  ...
  for (let i = 1; i <= a.length; i++) {
    for (let j = 1; j <= b.length; j++) {
      ...
      out.push(
        tableStep(
          `${a[i - 1]} -> ${b[j - 1]} costs ${cost}; cell = ${rows[i][j]}`,
          makeTable(
            rows,
            ["-", ...a],
            ["-", ...b],
            "Choose delete, insert, or substitute/match.",
            [[i, j]],
            [...done],
          ),
        ),
...
function runMatrix(size = 8, options = {}) {
  ...
  const p = core
    ? core.dimensions
    : [10, 30, 5, 60, 12, 24, 8, 16].slice(
        0,
        Math.max(4, Math.min(8, Math.round(size / 2))),
      );
  const n = p.length - 1;
  ...
  const out = [
    tableStep(
      "Single matrix intervals cost 0",
      makeTable(
        rows,
        labels,
        labels,
  ...
  for (let len = 2; len <= n; len++) {
    for (let i = 0; i <= n - len; i++) {
      ...
      for (let k = i; k < j; k++) {
        best = Math.min(
          best,
          rows[i][k] + rows[k + 1][j] + p[i] * p[k + 1] * p[j + 1],
        );
      }
      rows[i][j] = best;
      done.push([i, j]);
      out.push(
        tableStep(
          `Best cost for A${i + 1}..A${j + 1} is ${best}`,
          makeTable(
            rows,
            labels,
            labels,
            "Upper triangle stores interval costs.",
            [[i, j]],
            [...done],
          ),
        ),
...
function runLis(size = 8, options = {}) {
  ...
  const out = [
    tableStep(
      "Each position starts as a subsequence of length 1",
      makeTable(
        [arr, dp],
        ["value", "LIS"],
        arr.map((_, i) => i),
        "dp[i] is best increasing subsequence ending at i.",
        [],
        done,
      ),
    ),
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    out.push(
      tableStep(
        `Best subsequence ending at ${arr[i]} has length ${dp[i]}`,
        makeTable(
          [arr, dp],
          ["value", "LIS"],
          arr.map((_, k) => k),
          "Look left for smaller values and extend the best one.",
          [
            [0, i],
            [1, i],
          ],
