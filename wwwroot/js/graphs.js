...
const GRAPH_ALGORITHMS = {
  bfs: {
    ...
    sample: "unweighted",
    description:
      "Explores a graph in layers using a queue. It is the standard way to find shortest paths in unweighted graphs.",
    pseudocode:
  ...
  dfs: {
    ...
    sample: "unweighted",
    description:
      "Explores as far as possible before backtracking. DFS powers cycle checks, topological sort, and component algorithms.",
    pseudocode:
      "dfs(u):\n  mark u visited\n  for each neighbor v of u:\n    if v is unvisited:\n      parent[v] = u\n      dfs(v)",
  },
  topo: {
    ...
    sample: "dag",
    description:
      "Orders directed acyclic graph vertices so every dependency appears before the work that depends on it.",
    pseudocode:
  ...
  dijkstra: {
    ...
    time: "O((V + E) log V)",
    space: "O(V)",
    kind: "Nonnegative weights",
    use: "Routing",
    sample: "weighted",
    description:
      "Repeatedly settles the nearest unsettled vertex and relaxes outgoing edges. It requires nonnegative edge weights.",
    pseudocode:
  ...
  bellman: {
    ...
    time: "O(VE)",
    space: "O(V)",
    kind: "Negative weights",
    use: "Robust routing",
    sample: "negative",
    description:
      "Relaxes every edge repeatedly, which allows negative edges and exposes negative cycles.",
    pseudocode:
  ...
  floyd: {
    ...
    sample: "matrix",
    description:
      "Builds all-pairs shortest paths by allowing each vertex to become an intermediate waypoint.",
    pseudocode:
  ...
  prim: {
    ...
    time: "O(E log V)",
    space: "O(V)",
    kind: "Weighted undirected",
    use: "Networks",
    sample: "weighted",
    description:
      "Grows one tree by repeatedly choosing the cheapest edge that connects the current tree to a new vertex.",
    pseudocode:
      "start with source in the tree\nwhile tree does not contain all vertices:\n  choose minimum edge leaving the tree\n  add that edge and vertex to the tree",
  },
  kruskal: {
    name: "Kruskal MST",
    category: "Spanning tree",
    ...
    sample: "weighted",
    description:
      "Sorts edges by weight and accepts each edge that connects two different components.",
    pseudocode:
  ...
  unionFind: {
    ...
    sample: "weighted",
    description:
      "Maintains dynamic components with find and union operations. Kruskal uses it to decide whether an edge creates a cycle.",
    pseudocode:
  ...
  scc: {
    ...
    space: "O(V)",
    kind: "Directed",
    use: "Mutual reachability",
    sample: "scc",
    description:
      "Groups directed vertices that can all reach each other. This visualization follows Kosaraju-style finish and collection passes.",
    pseudocode:
  ...
  astar: {
    ...
    space: "O(V)",
    kind: "Weighted spatial",
    use: "Pathfinding",
    sample: "weighted",
    description:
      "Chooses vertices by distance so far plus an admissible heuristic estimate. This demo uses h=0, so it remains exact and behaves like Dijkstra on the sample.",
    pseudocode:
...
function generatedNode(index, total) {
  ...
  return n(
    nodeLabel(index),
    Math.round(410 + Math.cos(angle) * 300),
    Math.round(205 + Math.sin(angle) * 145),
  );
}
function randomizedNode(index, total) {
  ...
  return n(
    node.id,
    Math.max(55, Math.min(765, node.x + randInt(-38, 38))),
    Math.max(55, Math.min(365, node.y + randInt(-28, 28))),
  );
}
...
function resizeGraphSample(sample, size) {
  ...
  const nodes = Array.from({ length: nextSize }, (_, i) =>
    randomizedNode(i, nextSize),
  );
  const edges = [];
  ...
  const isDag =
    sample.directed &&
    (hasNegative ||
      (!weighted && sample.edges.every((edge) => edge.from < edge.to)));
  ...
  if (sample.directed && !weighted && !isDag) {
    ...
    for (let i = 0; i < split; i++)
      addEdge(nodeLabel(i), nodeLabel((i + 1) % split));
    for (let i = split; i < nextSize; i++)
      addEdge(nodeLabel(i), nodeLabel(i + 1 < nextSize ? i + 1 : split));
...
function neighbors(graph, id) {
  ...
  for (const edge of graph.edges) {
    ...
    else if (!graph.directed && edge.to === id)
      out.push({ id: edge.from, edge });
  }
  return out;
}
...
function renderGrid() {
  refs.grid.innerHTML = Object.entries(GRAPH_ALGORITHMS)
    .map(
      ([key, algo]) => `
...
function fillSelectors(graph) {
  ...
  refs.source.innerHTML = ids
    .map((id) => `<option value="${id}">${id}</option>`)
    .join("");
  refs.target.innerHTML = ids
    .map((id) => `<option value="${id}">${id}</option>`)
    .join("");
...
function buildSteps() {
  ...
  const resultSteps = (ALGO_RUNNERS[currentKey] || runBfs)(
    currentGraph,
    source,
    target,
  );
  steps = [
    step("Queued. Press Play to start."),
    ...(resultSteps.length ? resultSteps : [step("Ready")]),
  ];
  stepIndex = 0;
  renderStep(steps[0]);
}
function rebuildGraph() {
  ...
  refs.target.value = ids.includes(previousTarget)
    ? previousTarget
    : ids[ids.length - 1];
  buildSteps();
}
...
function renderStep(item) {
  ...
  refs.visited.textContent = new Set([
    ...(item.state.visited || []),
    ...(item.state.path || []),
  ]).size;
...
function renderGraph(state) {
  ...
  const marker = currentGraph.directed
    ...
    : "";
  const edgeLines = currentGraph.edges
    .map((edge) => {
      const a = nodeMap.get(edge.from);
      const b = nodeMap.get(edge.to);
      const id = edgeId(edge, currentGraph);
      const cls = state.edgeRejected.includes(id)
        ? "vs-graph-edge-rejected"
        : state.edgePath.includes(id)
          ? "vs-graph-edge-path"
          : state.edgeFrontier.includes(id)
            ? "vs-graph-edge-frontier"
            : "";
      const mx = (a.x + b.x) / 2;
      const my = (a.y + b.y) / 2;
      const markerEnd = currentGraph.directed
  ...
  const nodes = currentGraph.nodes
    .map((node) => {
      const cls = state.path.includes(node.id)
        ? "vs-graph-node-path"
        : state.current === node.id
          ? "vs-graph-node-current"
          : state.frontier.includes(node.id)
            ? "vs-graph-node-frontier"
            : state.visited.includes(node.id)
              ? "vs-graph-node-visited"
              : "";
...
function runBfs(graph, source, target) {
  ...
  while (q.length) {
    const u = q.shift();
    out.push(
      step(`Visit ${u}; queue: ${q.join(", ") || "empty"}`, {
        current: u,
        visited: [...seen],
        frontier: q,
      }),
    ...
    for (const nb of neighbors(graph, u)) {
      ...
      q.push(nb.id);
      out.push(
        step(`Discover ${nb.id} from ${u}`, {
          current: u,
          visited: [...seen],
          frontier: q,
          edgeFrontier: [edgeId(nb.edge, graph)],
        }),
      );
    }
  }
  ...
  out.push(
    step(
      path.path.includes(target)
        ? `Shortest unweighted path: ${path.path.join(" -> ")}`
        : `${target} is unreachable`,
      { visited: [...seen], path: path.path, edgePath: path.edgePath },
    ),
  );
  return out;
}
...
function runTopo(graph) {
  ...
  const order = [];
  const out = [
    step(`Initial indegree-0 queue: ${q.join(", ")}`, { frontier: q }),
  ];
  while (q.length) {
    const u = q.shift();
    order.push(u);
    out.push(
      step(`Output ${u}`, { current: u, visited: [...order], frontier: q }),
    );
    for (const nb of neighbors(graph, u)) {
      indeg[nb.id]--;
      ...
      out.push(
        step(
          `Remove edge ${u} -> ${nb.id}; indegree(${nb.id}) = ${indeg[nb.id]}`,
          {
            current: u,
            visited: [...order],
            frontier: q,
...
function runDijkstra(graph, source, target) {
  ...
  while (settled.size < ids.length) {
    ...
    settled.add(u);
    out.push(
      step(`Settle ${u} with distance ${dist[u]}`, {
        current: u,
        visited: [...settled],
      }),
    );
  ...
  out.push(
    step(
      path.path.includes(target)
        ? `Shortest path cost ${dist[target]}: ${path.path.join(" -> ")}`
        : `${target} is unreachable`,
      { visited: [...settled], path: path.path, edgePath: path.edgePath },
    ),
  );
  return out;
}
  ...
  for (let pass = 1; pass < ids.length; pass++) {
    ...
    for (const edge of graph.edges) {
      if (
        ...
        dist[edge.from] + weight(edge) < dist[edge.to]
      ) {
        ...
        out.push(
          step(
            `Pass ${pass}: relax ${edge.from} -> ${edge.to}; dist = ${dist[edge.to]}`,
            {
              current: edge.from,
              frontier: [edge.to],
              edgeFrontier: [edgeId(edge, graph)],
            },
          ),
        );
      }
  ...
  out.push(
    step(
      negativeCycle
        ...
        : `Bellman-Ford result for ${target}: ${dist[target]}`,
      {
        path: negativeCycle ? [] : path.path,
        edgePath: negativeCycle ? [] : path.edgePath,
      },
    ),
  );
  return out;
    ...
    for (const i of ids) {
      for (const j of ids) {
        if (dist[i][k] + dist[k][j] < dist[i][j]) {
          dist[i][j] = dist[i][k] + dist[k][j];
          out.push(
            step(`Use ${k}: improve ${i} -> ${j} to ${dist[i][j]}`, {
              current: k,
              frontier: [i, j],
            }),
          );
...
function runPrim(graph, source) {
  ...
  while (tree.size < graph.nodes.length) {
    const candidates = graph.edges.filter(
      (edge) => tree.has(edge.from) !== tree.has(edge.to),
    );
    ...
    out.push(
      step(`Choose edge ${best.from}-${best.to} weight ${weight(best)}`, {
        current: next,
        visited: [...tree],
        edgePath: [...edgePath],
      }),
  ...
  out.push(
    step("Minimum spanning tree complete", { visited: [...tree], edgePath }),
  );
  return out;
}
...
function runKruskal(graph) {
  ...
  for (const edge of [...graph.edges].sort((a, b) => weight(a) - weight(b))) {
    if (dsu.union(edge.from, edge.to)) {
      ...
      out.push(
        step(`Accept ${edge.from}-${edge.to}`, { edgePath: [...chosen] }),
      );
    } else {
      rejected.push(edgeId(edge, graph));
      out.push(
        step(`Reject ${edge.from}-${edge.to}; it forms a cycle`, {
          edgePath: [...chosen],
  ...
  out.push(
    step("Kruskal MST complete", { edgePath: chosen, edgeRejected: rejected }),
  );
  return out;
}
...
function runUnionFind(graph) {
  ...
  for (const edge of graph.edges) {
    if (dsu.union(edge.from, edge.to)) {
      chosen.push(edgeId(edge, graph));
      out.push(
        step(`union(${edge.from}, ${edge.to}) merges two sets`, {
          current: edge.from,
          frontier: [edge.to],
          edgePath: [...chosen],
        }),
      ...
      rejected.push(edgeId(edge, graph));
      out.push(
        step(`find(${edge.from}) equals find(${edge.to}); skip`, {
          current: edge.from,
          frontier: [edge.to],
          edgePath: [...chosen],
          edgeRejected: [...rejected],
...
function runScc(graph) {
  ...
  function dfs1(u) {
    seen.add(u);
    out.push(
      step(`Visit ${u} in first pass`, { current: u, visited: [...seen] }),
    );
    for (const nb of neighbors(graph, u)) if (!seen.has(nb.id)) dfs1(nb.id);
    order.push(u);
    out.push(
      step(`Push ${u} by finish time`, { current: u, path: [...order] }),
    );
  }
  nodeIds(graph).forEach((id) => {
    if (!seen.has(id)) dfs1(id);
  });
  const rev = cloneGraph(graph);
  rev.edges = rev.edges.map((edge) => ({
    from: edge.to,
    to: edge.from,
    weight: edge.weight,
  }));
  ...
  while (order.length) {
    ...
    dfs2(start);
    out.push(
      step(`Component ${component}: ${group.join(", ")}`, { path: group }),
    );
  }
  return out;
}
...
function runAstar(graph, source, target) {
  ...
  const out = [
    step(`Start A* at ${source}; target ${target}`, { frontier: [source] }),
  ];
  while (open.size) {
    ...
    for (const nb of neighbors(graph, u)) {
      ...
      if (tentative < g[nb.id]) {
        ...
        out.push(
          step(
            `Improve ${nb.id}; g = ${g[nb.id]}, f = ${f[nb.id].toFixed(2)}`,
            {
              current: u,
              visited: [...closed],
              frontier: [...open],
  ...
  out.push(
    step(
      path.path.includes(target)
        ? `A* path: ${path.path.join(" -> ")}`
        : `${target} is unreachable`,
      { visited: [...closed], path: path.path, edgePath: path.edgePath },
    ),
  );
  return out;
}
