...
const TREE_DATA = {
  bst: {
    ...
    space: "O(n)",
    description:
      "Stores smaller keys to the left and larger keys to the right. It is the baseline search tree that makes every later balancing idea easier to see.",
    pseudocode:
  ...
  avl: {
    ...
    space: "O(n)",
    description:
      "Keeps each node height-balanced with rotations so every balance factor remains between -1 and 1.",
    pseudocode:
  ...
  rb: {
    ...
    space: "O(n)",
    description:
      "Uses node colors and local rotations to keep the root black, avoid red-red links, and bound tree height.",
    pseudocode:
  ...
  splay: {
    ...
    space: "O(n)",
    description:
      "Moves recently accessed keys toward the root using zig, zig-zig, and zig-zag rotations.",
    pseudocode:
  ...
  treap: {
    ...
    space: "O(n)",
    description:
      "Combines BST ordering by key with heap ordering by deterministic demo priorities.",
    pseudocode:
  ...
  twoThree: {
    ...
    space: "O(n)",
    description:
      "Stores one or two keys per node and keeps every leaf at the same depth. Splits are visible as multi-key nodes settle back into order.",
    pseudocode:
  ...
  twoThreeFour: {
    ...
    space: "O(n)",
    description:
      "A B-tree of minimum degree 2. Nodes may hold one, two, or three keys, matching red-black tree behavior from another angle.",
    pseudocode:
  ...
  btree: {
    ...
    space: "O(n)",
    description:
      "A broad multiway tree tuned for block storage. This visualizer uses minimum degree 3 so node splits are easy to inspect.",
    pseudocode:
  ...
  bplus: {
    ...
    space: "O(n)",
    description:
      "Keeps records in ordered leaves with separator keys above them for multiway search.",
    pseudocode:
...
function randomKeys(size, min = 5, max = 250) {
  const keys = new Set();
  while (keys.size < size && keys.size < max - min + 1)
    keys.add(Math.floor(Math.random() * (max - min + 1)) + min);
  return [...keys];
}
...
class BinaryModel {
  ...
  insert(key) {
    ...
    out.push(
      this.snapshot(
        { change: [key] },
        exists ? `${key} is already present` : `Inserted ${key}`,
      ),
    );
    return out;
  }
  ...
  search(key) {
    ...
    return [
      ...
      this.snapshot(
        found ? { found: [key] } : { change: path.slice(-1) },
        found ? `Found ${key}` : `${key} is not in the tree`,
      ),
    ];
  }
  ...
  delete(key) {
    const path = this.pathTo(key);
    const out = [
      this.snapshot({ visit: path }, `Search path for deleting ${key}`),
    ];
    if (this.keys.includes(key)) {
      ...
      out.push(
        this.snapshot({ change: path.slice(-1) }, `${key} was not found`),
      );
    }
    return out;
  }
  ...
  predecessor(key) {
    return [...this.keys].reverse().find((k) => k < key);
  }
  successor(key) {
...
class MultiwayModel {
  ...
  levels() {
    ...
    for (let i = 0; i < this.keys.length; i += this.maxKeys) {
      leaves.push({
        id: `l${i}`,
        keys: this.keys.slice(i, i + this.maxKeys),
        children: [],
      });
    }
  ...
  insert(key) {
    const exists = this.keys.includes(key);
    const out = [
      this.snapshot({ visit: [key] }, `Descend to the leaf for ${key}`),
    ];
    if (!exists) this.insertSilent(key);
    out.push(
      this.snapshot(
        ...
        exists
          ...
          : `Inserted ${key}; split/promote if a node overflowed`,
      ),
    );
    return out;
  }
  ...
  search(key) {
    ...
    return [
      this.snapshot(
        { visit: [key] },
        `Compare separators while searching for ${key}`,
      ),
      this.snapshot(
        found ? { found: [key] } : { change: [key] },
        found ? `Found ${key} in a leaf` : `${key} is not in the tree`,
      ),
    ];
  }
  ...
  delete(key) {
    const out = [
      this.snapshot({ visit: [key] }, `Find the leaf for deleting ${key}`),
    ];
    if (this.keys.includes(key)) {
      this.removeSilent(key);
      out.push(
        this.snapshot(
          { change: [key] },
          `Deleted ${key}; borrow/merge if a node underflowed`,
        ),
      );
    } else {
  ...
  traversal(type) {
    ...
    return keys.map((key, i) =>
      this.snapshot(
        { found: keys.slice(0, i + 1), visit: [key] },
        `${type}: ${keys.slice(0, i + 1).join(", ")}`,
      ),
    );
  }
}
...
function renderGrid() {
  refs.grid.innerHTML = Object.entries(TREE_DATA)
    .map(
      ([key, tree]) => `
...
function createModel(key) {
  ...
  if (globalThis.AlgorithmCore)
    return globalThis.AlgorithmCore.trees.createSession(key, {
      size,
      randomize: true,
    });
...
function createModelFromKeys(key, keys) {
  if (globalThis.AlgorithmCore)
    return globalThis.AlgorithmCore.trees.createSession(key, [...keys]);
  if (["twoThree", "twoThreeFour", "btree", "bplus"].includes(key)) {
    const maxKeys = key === "btree" || key === "bplus" ? 5 : 3;
...
function keysAfter(type, key) {
  ...
  if (type === "search") {
    return currentTreeKey === "splay" && keys.includes(key)
      ? keys.filter((k) => k !== key).concat(key)
      : keys;
  }
  if (type === "insert") {
...
function minMaxPath(direction) {
  const snap = model.snapshot();
  if (snap.kind === "multiway")
    return direction === "min" ? [model.min()] : [model.max()];
  const path = [];
  let node = snap.root;
  while (node) {
    path.push(node.key);
...
function renderMultiway(snapshot) {
  ...
  all.forEach((node) => {
    ...
    node.children.forEach((child) => {
      parts.push(
        `<line class="vs-tree-edge" x1="${node._x}" y1="${node._y + 24}" x2="${child._x}" y2="${child._y - 24}"></line>`,
      );
    });
  });
  for (const node of all) {
    const keyText = node.keys.join(" | ");
    const state = node.keys.some(
      (k) => snapshot.highlight.found && snapshot.highlight.found.includes(k),
    )
      ? "vs-tree-node-found"
      : node.keys.some(
            (k) =>
              snapshot.highlight.change &&
              snapshot.highlight.change.includes(k),
          )
        ? "vs-tree-node-change"
        : node.keys.some(
              (k) =>
                snapshot.highlight.visit &&
                snapshot.highlight.visit.includes(k),
            )
          ? "vs-tree-node-visit"
          : "";
    const w = Math.max(66, 26 * node.keys.length + 22);
    parts.push(
      `<g class="vs-tree-node ${state}" transform="translate(${node._x} ${node._y})"><rect x="${-w / 2}" y="-24" width="${w}" height="48" rx="8"></rect><text>${keyText}</text></g>`,
    );
  }
...
function nodeState(key, highlight) {
  if (highlight.found && highlight.found.includes(key))
    return "vs-tree-node-found";
  if (highlight.change && highlight.change.includes(key))
    return "vs-tree-node-change";
  if (highlight.visit && highlight.visit.includes(key))
    return "vs-tree-node-visit";
  return "";
}
...
function runOperation(type) {
  ...
  if (advanced && type !== "search" && found !== (type === "insert")) {
    opSteps.push(
      snapshotForKeys(
        nextKeys,
        { change: [key] },
        `${TREE_DATA[currentTreeKey].name} rebalance checkpoint`,
      ),
    );
  }
  ...
  const finalHighlight =
    type === "search" && found
      ? { found: [key] }
      : { change: found || type === "insert" ? [key] : path.slice(-1) };
  const finalMessage = {
  ...
  const probe =
    label === "Min"
      ...
      : label === "Max"
        ...
        : searchPath(keyValue());
  const msg =
    value === undefined ? `${label} does not exist` : `${label}: ${value}`;
  setSteps([
    model.snapshot({}, `${label} queued. Press Play to start.`),
    model.snapshot(
      { visit: probe.filter((v) => v !== undefined) },
      `Trace ${label.toLowerCase()} path`,
    ),
    model.snapshot(
      value === undefined ? { change: probe.slice(-1) } : { found: [value] },
      msg,
    ),
  ]);
}
...
document
  .getElementById("tree-insert")
  .addEventListener("click", () => runOperation("insert"));
document
  .getElementById("tree-search")
  .addEventListener("click", () => runOperation("search"));
document
  .getElementById("tree-delete")
  .addEventListener("click", () => runOperation("delete"));
document
  .getElementById("tree-min")
  .addEventListener("click", () => runQuery("Min", model.min()));
document
  .getElementById("tree-max")
  .addEventListener("click", () => runQuery("Max", model.max()));
document
  .getElementById("tree-prev")
  .addEventListener("click", () =>
    runQuery("Predecessor", model.predecessor(keyValue())),
  );
document
  .getElementById("tree-next")
  .addEventListener("click", () =>
    runQuery("Successor", model.successor(keyValue())),
  );
document
  .querySelectorAll("[data-tree-traversal]")
  .forEach((btn) =>
    btn.addEventListener("click", () =>
      runTraversal(btn.dataset.treeTraversal),
    ),
  );
