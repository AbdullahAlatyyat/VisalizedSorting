...
procedure merge(left, right):
  ...
  quick: {
    ...
    pseudocode: `procedure quickSort(arr, lo, hi):
  if lo < hi then
...
procedure heapify(arr, n, root):
  ...
  counting: {
    ...
    pseudocode: `procedure countingSort(arr, k):
...
function renderHeroBars() {
  ...
  const bars = [
    { h: 10, type: "sorted", dur: 3.2, delay: 0.0, to: 0.9 },
    ...
    { h: 41, type: "sorted", dur: 3.6, delay: 0.3, to: 0.9 },
    { h: 50, type: "sorted", dur: 3.3, delay: 0.5, to: 0.88 },
    { h: 58, type: "sorted", dur: 3.2, delay: 0.2, to: 0.92 },
    { h: 76, type: "compare", dur: 1.0, delay: 0.0, to: 0.6 },
    { h: 62, type: "compare", dur: 1.0, delay: 0.12, to: 0.6 },
    { h: 88, type: "default", dur: 2.0, delay: 0.35, to: 0.4 },
    { h: 44, type: "default", dur: 1.7, delay: 0.1, to: 0.5 },
    { h: 72, type: "default", dur: 2.3, delay: 0.55, to: 0.35 },
    { h: 55, type: "default", dur: 1.9, delay: 0.25, to: 0.45 },
    { h: 94, type: "default", dur: 2.1, delay: 0.4, to: 0.3 },
    { h: 34, type: "default", dur: 1.6, delay: 0.15, to: 0.55 },
    { h: 80, type: "default", dur: 2.4, delay: 0.6, to: 0.38 },
    { h: 48, type: "default", dur: 1.8, delay: 0.3, to: 0.48 },
...
function renderAlgoGrid() {
  algoGrid.innerHTML = Object.entries(ALGORITHMS)
    .map(([key, algo]) => {
      const badgeClass =
...
<div class="vs-algo-card" data-key="${key}" role="button" tabindex="0"
  ...
  algoGrid.addEventListener("keydown", (e) => {
    if (e.key === "Enter" || e.key === " ") {
      ...
      if (card) {
        e.preventDefault();
        openVisualizer(card.dataset.key);
      }
    }
  });
}
...
function* quickSortGen(arr) {
  ...
  const unsorted = Array.from({ length: arr.length }, (_, k) => k).filter(
    (k) => !sorted.has(k),
  );
...
const Engine = (() => {
  ...
  function _syncPlayBtn() {
    btnPlay.innerHTML = _playing ? PAUSE_SVG + " Pause" : PLAY_SVG + " Play";
  }
...
sizeSlider.addEventListener("input", () => {
  ...
  if (currentAlgoKey)
    Engine.reset(currentAlgoKey, parseInt(sizeSlider.value, 10));
});
...
backBtn.addEventListener("click", () =>
  closeVisualizer({ scrollToGrid: true }),
);
